# C-MPS
- Cracko298's Modded Performance Shaders (C-MPS).
- Is a Shader-Pack that optimizes the absolutely terrible Shader assembly of the original MC3DS Shaders.

## Feature(s):
- Small footprint, most of these shaders are alot smaller than MC3DS' BaseGame Shaders.
- Simpler Instructions, and optimized functions.
- Resulting in... *More FPS*.
  - Testing shows the game hangs above 60+ FPS. Instead of Around ~40 FPS.
  - Occasional drops, but that's presumably because of the GPU.
